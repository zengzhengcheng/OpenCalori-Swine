        ...
        try:
            if (not qreuslt.empty):
                ...
                if (not head):
                    ...
                    if (vd.ac == 0):
                        print(rowindex)
                    if(vd.lastac is not None):
                        if(vd.ac>vd.lastac):
                            jingqimessage[sntime]=i
                        vd["lastac"] = vd.ac
                    vd.ag = vd.af / vd.ac
                    if(vd.ag<0.6 or vd.ag>1.2):
...
class MainWindow(QMainWindow,Ui_MainWindow):
    def __init__(self,parent=None):
        ...
        self.pushButton_selectswall.clicked.connect(self.getSWAllFile)
        self.pushButton_selectsnall.clicked.connect(self.getSNAllFile)
        self.pushButton.clicked.connect(self.write)
        self.pushButton_xiugai.clicked.connect(self.xiugaijiange)
        self.jiange=30
    def getSWAllFile(self):
        folder_path = QFileDialog.getExistingDirectory(self, "选择文件夹", "./")  # 默认打开 C 盘
        df_list=[]
        if folder_path:
            # 递归遍历所有.xls文件（不区分大小写）
            for idx, file_path in enumerate(Path(folder_path).rglob("**/*.[xX][lL][sS]"), 1):
                abs_path = str(file_path.resolve())  # 转换为绝对路径
                df_list.append(self.getSWFileData(abs_path))
    ...
    def getSNAllFile(self):
        ...
        if folder_path:
            for idx, file_path in enumerate(Path(folder_path).rglob("**/*.[xX][lL][sS]"), 1):
                abs_path = str(file_path.resolve())  # 转换为绝对路径
                df_list.append(self.getSNFileData(abs_path))
            df=pd.concat(df_list)
            df=df.sort_values(by="sntime")
            self.dsn=df
    ...
    def getSNFilePath(self):
        ...
        datasn.dropna(axis=0, how="any", inplace=True)
        first = True
        dsn = None
        for i in datasn:
    ...
    def write(self):
        ...
        shangmessage=message[2]
        jingqimessage=message[3]
        allmessage=set()
        for i in tubianmessage.keys():
            allmessage.add(i)
        for i in shangmessage.keys():
            allmessage.add(i)
        for i in jingqimessage.keys():
            allmessage.add(i)
        if fname[0]:  # 如果获取的路径非空
            ...
            for i in allmessage:
                sheet_table.write(str(i), 0, str(i), workfomat)
            workbook.close()
            writer.close()
            mname=name[:-5]+".txt"
